...
export class MemberDetailComponent implements OnInit, OnDestroy{
  ...
  ngOnInit(): void {
    this.route.data.subscribe({
      next: data => {this.member = data['member']; this.member && this.member.photos.map(p => {this.images.push(new ImageItem({src: p.url, thumb: p.url}))})}
    });
  ...
  onRouteParamsChange(){
    ...
    if (this.messageService.hubConnection?.state === HubConnectionState.Connected && this.activeTab?.heading === 'Messages') {
      this.memberService.hubConnection.stop().then(() => {
  ...
  onTabActivated(data: TabDirective){
    ...
    this.router.navigate([], {
      relativeTo: this.route, queryParams: {tab: this.activeTab.heading}, queryParamsHandling: 'merge'
    })
